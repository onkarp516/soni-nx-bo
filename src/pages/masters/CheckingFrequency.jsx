...
import { DTCheckingFrequencyUrl } from "@/services/api";
import {
  createCheckingFrequency,
  findCheckingFrequency,
  updateCheckingFrequency,
  deleteCheckingFrequency,
  ...
  return (
    <>
      {currentIndex === rowIndex ? (
        ...
        isActionExist("checking-frequency", "edit", userPermissions) && (
          <Button
            title="EDIT"
            id="Tooltipedit"
      ...
      {isActionExist("checking-frequency", "delete", userPermissions) && (
        <Button
          title="DELETE"
          id="Tooltipdelete"
...
function CheckingFrequency(props) {
  const [addModalShow, setAddModalShow] = useState(false);
  const [editModalShow, setEditModalShow] = useState(false);
  const [frequencyObject, setFrequencyObject] = useState("");
  ...
  const columns = [
    {
      id: "checkingFrequencyLabel",
      field: "checkingFrequencyLabel",
      label: "Heading",
      resizable: true,
    },
    // {
  ...
  const addBtn = isWriteAuthorized("master", "designationPermission") &&
    isActionExist("checking-frequency", "create", props.userPermissions) && (
      <>
        <button
          title="CREATE"
          onClick={(e) => {
  ...
  const onEditModalShow = (status, id, rowIndex) => {
    if (status) {
      ...
      findCheckingFrequency(reqData)
        .then((response) => {
          if (response.data.responseStatus == 200) {
            setFrequencyObject(response.data.response);
            setcurrentIndex(0);
  ...
  const onDeleteModalShow = (id) => {
    Swal.fire({
      ...
      deleteCheckingFrequency(reqData)
        .then((response) => {
          if (response.data.responseStatus == 200) {
  ...
  return (
    <div>
      ...
      >
        ...
        >
          Create Checking Frequency
        </ModalHeader>
        <Formik
          validateOnBlur={false}
          validateOnChange={false}
          initialValues={{
            checkingFrequencyLabel: "",
          }}
          validationSchema={Yup.object().shape({
            checkingFrequencyLabel: Yup.string()
              .trim()
          ...
          onSubmit={(values, { resetForm, setStatus, setSubmitting }) => {
            ...
            requestData.append(
              "checkingFrequencyLabel",
              values.checkingFrequencyLabel
            );
            createCheckingFrequency(requestData)
              .then((response) => {
                setIsLoading(false);
                if (response.data.responseStatus === 200) {
                  setSubmitting(false);
            ...
            <Form autoComplete="off">
              <ModalBody>
                <Row>
                  <Col md="4">
                    <FormGroup>
                      <Label>Checking Frequency Heading</Label>
                      <Input
                        type="text"
                        placeholder="Enter Heading"
                        name="checkingFrequencyLabel"
                        onChange={handleChange}
                        value={values.checkingFrequencyLabel}
                        invalid={errors.checkingFrequencyLabel ? true : false}
                      />
                      <FormFeedback>
                        {errors.checkingFrequencyLabel}
                      </FormFeedback>
                    </FormGroup>
                  </Col>
                </Row>
        ...
        <Formik
          ...
          initialValues={{
            checkingFrequencyLabel:
              frequencyObject != null
                ? frequencyObject.checkingFrequencyLabel
                : "",
          }}
          validationSchema={Yup.object().shape({
            checkingFrequencyLabel: Yup.string()
          ...
          onSubmit={(values, { resetForm, setStatus, setSubmitting }) => {
            ...
            let requestData = {
              ...
              checkingFrequencyLabel: values.checkingFrequencyLabel,
            };

            updateCheckingFrequency(requestData)
              .then((response) => {
                setIsLoading(false);
                if (response.data.responseStatus === 200) {
                  setSubmitting(false);
...
export default WithUserPermission(CheckingFrequency);
